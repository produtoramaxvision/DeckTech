...
COM loop-only (cold): 182 shortcuts, 267.2 ms total, 1.47 ms/shortcut
COM wall incl. PowerShell startup + COM instantiation (cold): 649.4 ms total
Node binary parser (cold): 182 shortcuts, 28.04 ms total, 0.1541 ms/shortcut
Speedup (cold, COM loop-only / Node parser): 9.5x
...
Node parser: median 21.59 ms, min 18.43, max 28.11, stddev 3.89 -- raw: [25.97, 28.11, 21.59, 18.67, 18.43]
COM loop-only: median 289.6 ms, min 250.6, max 387.4, stddev 45.8 -- raw: [387.4, 311.7, 289.6, 283.9, 250.6]
Speedup (median of per-iteration COM/Node ratios): median 13.6x, min 11.1x, max 15.2x
```

Reading this honestly: the Node parser is consistently faster than COM by
roughly an order of magnitude, both cold (9.5x) and warm (median 13.6x,
range 11.1x–15.2x across 5 iterations). It is NOT a stable "8.0x" or any
other single two-significant-figure number — that was round 1's mistake
(n=1, no variance reported). COM's own loop-only time varies by nearly 2x
across warm iterations (250.6–387.4 ms) on this machine, which is itself
evidence for why n=1 was insufficient. Warming does not make COM
consistently faster than its cold figure here (median 289.6 ms warm vs.
267.2 ms cold) — the file-cache-warmth explanation round 1 offered for why
COM might read fast was not borne out by repetition; COM's variance appears
dominated by something other than file-cache state (plausibly PowerShell
process / COM instantiation jitter, not investigated further here).
...
ADR's own speedup claims (9.5x cold, 11.1x–15.2x warm) are computed against
...
number anyway: this run's warm-median Node parser time is 21.59 ms for 182
shortcuts (0.1187 ms/shortcut); dividing the original 16.07 ms/shortcut
baseline by that gives ~135x, but that figure inherits every ambiguity in
the 149 denominator above and is not asserted as a validated speedup.
...
through to `env-expanded` rather than `linkinfo-local`). Round 1 asserted
"none tripped `ForceNoLinkInfo`" with no field in the report to check it
against; that claim was simply wrong, not merely unverifiable (round-2
minor finding 9).
...
2. A roughly-order-of-magnitude speedup over COM, measured with warmup and
   repetition: 9.5x cold (n=1), median 13.6x warm (n=5, range 11.1x–15.2x).
