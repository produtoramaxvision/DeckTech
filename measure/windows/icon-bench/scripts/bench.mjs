...
// Round-8 review fix (finding 2, major): `--allow-incomplete` is the explicit
// opt-in required to benchmark an apps.json whose population is known
// incomplete (list-apps.mjs recorded at least one unreadable Start Menu
// subdirectory). Without this flag, an incomplete population is now fatal
// here — see the populationComplete gate below — instead of silently
// benchmarking a shrunken set with a clean exit code.
...
// Round-8 review fix (finding 2, major): unreadableDirCount/populationComplete
// used to be write-only — list-apps.mjs recorded them, but nothing downstream
// ever read them, so a subdirectory-level EPERM during enumeration could
// silently shrink the population all the way into a "clean" benchmark run.
// This is the load-bearing gate: an apps.json whose population is
// (a) missing the stamp entirely (generated by a pre-round-8 list-apps.mjs —
//     treated as incomplete/unknown, NOT as complete, so an old cached
//     apps.json can't silently bypass this gate by predating it), or
// (b) explicitly incomplete (populationComplete === false)
// is now fatal unless the caller passes --allow-incomplete. The flag exists
// so an incomplete population can still be benchmarked, but only by
// deliberate choice, never by default.
...
if (!populationComplete) {
  ...
  const banner = `[bench] *** INCOMPLETE POPULATION *** apps.json.populationComplete=${JSON.stringify(appsData.populationComplete)} (unreadableDirCount=${unreadableDirCount}) — the benchmarked app set may be smaller than the real Start Menu contents.`;
  if (!ALLOW_INCOMPLETE) {
    console.error(banner);
    console.error("[bench] FATAL: refusing to benchmark an incomplete population. Re-run scripts/list-apps.mjs after fixing the unreadable directory, or pass --allow-incomplete to proceed deliberately (the incompleteness will be stamped into results.json and re-printed at the end).");
    process.exit(1);
  }
  console.error(banner);
  console.error("[bench] --allow-incomplete given: proceeding anyway. This is a deliberate opt-in, not a default.");
}
...
async function main() {
  const report = {
    ...
    appCount: N_APPS,
    iconSize: ICON_SIZE,
    measurePasses: MEASURE_PASSES,
    poolSize: POOL_SIZE,
    // Round-8 review fix (finding 2, major): stamped into results.json itself
    // (not just printed at load time), so anyone reading a results.json
    // later — without having seen the stdout banner above — can still tell
    // this run's population was known incomplete, and by how much.
    populationComplete,
  ...
  // Round-8 review fix (finding 2, major): re-print the incomplete-population
  // banner here too, not just at load time — the load-time banner scrolls
  // off above 300+ lines of per-candidate output by the time a reader reaches
  // the summary, and a banner nobody scrolls back to is functionally silent.
  if (!populationComplete) {
    console.log(`\n[bench] *** INCOMPLETE POPULATION *** this run used --allow-incomplete against an apps.json with populationComplete=${JSON.stringify(appsData.populationComplete)} (unreadableDirCountAtListTime=${report.unreadableDirCountAtListTime}). The numbers below do not reflect the full real app set. See results.json.populationComplete.`);
  }
  console.log("\n[bench] === SUMMARY TABLE ===");
